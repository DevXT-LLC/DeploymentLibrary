...
# On Jetson (ARM64 + CUDA), uses Docker (docker-compose-jetson.yml) to avoid dependency issues.
set -e
...
# ------------------------------------------------------------------
# 3b. Jetson Docker detection — skip venv/pip entirely
# ------------------------------------------------------------------
IS_JETSON=false
USE_JETSON_DOCKER=false
ARCH=$(uname -m)
if [ "$ARCH" = "aarch64" ] || [ "$ARCH" = "arm64" ]; then
    if [ -f /etc/nv_tegra_release ] || ([ -f /proc/device-tree/model ] && grep -qi "jetson\|tegra" /proc/device-tree/model 2>/dev/null); then
        IS_JETSON=true
        # Check if Docker + NVIDIA container runtime are available
        if command -v docker &>/dev/null; then
            if docker info 2>/dev/null | grep -qi "nvidia\|tegra"; then
                USE_JETSON_DOCKER=true
            elif [ -f /etc/nvidia-container-runtime/config.toml ] || \
...
if $USE_JETSON_DOCKER; then
    echo ""
    echo "Jetson detected with NVIDIA Docker runtime — using Docker deployment."
    echo "This avoids ARM64 dependency issues by building everything in a container."
    echo ""
    ...
    cat > "${ENV_FILE}" <<ENVEOF
# ezlocalai configuration - generated by deployment script (Jetson Docker mode)
...
ENVEOF
    ...
    # Auto-detect JetPack version for the correct L4T base image tag
    if [ -f /etc/nv_tegra_release ]; then
        L4T_VER=$(head -1 /etc/nv_tegra_release | sed 's/.*R\([0-9]*\).*(revision: \([0-9.]*\)).*/r\1.\2/' 2>/dev/null || echo "")
    fi
    if [ -z "$L4T_VER" ] && command -v dpkg &>/dev/null; then
        L4T_VER=$(dpkg -l nvidia-l4t-core 2>/dev/null | awk '/^ii/{print $3}' | sed 's/^\([0-9]*\.[0-9]*\).*/r\1/' || echo "")
    fi
    if [ -n "$L4T_VER" ]; then
        echo "Detected L4T version: ${L4T_VER}"
        # Map to known base image tags
        case "$L4T_VER" in
            r36*) export L4T_TAG="${L4T_VER}" ;;
            r35*) export L4T_TAG="${L4T_VER}" ;;
            *)    export L4T_TAG="r36.4.0" ;;  # default to latest
        esac
    else
        export L4T_TAG="r36.4.0"
    fi

    # Auto-detect CUDA architecture by Jetson model
    CUDA_ARCH=87  # default: Orin
    if [ -f /proc/device-tree/model ]; then
        MODEL_NAME=$(tr -d '\0' < /proc/device-tree/model 2>/dev/null || echo "")
        case "$MODEL_NAME" in
            *Xavier*)  CUDA_ARCH=72 ;;
            *Nano*)    CUDA_ARCH=53 ;;
            *Orin*)    CUDA_ARCH=87 ;;
        esac
        echo "Detected Jetson model: ${MODEL_NAME} (CUDA arch: sm_${CUDA_ARCH})"
    fi
    export CUDA_ARCH

    echo "Building Jetson Docker image (this may take 10-20 minutes on first run)..."
    cd "${INSTALL_DIR}"
    docker compose -f docker-compose-jetson.yml build

    echo "Starting ezlocalai in Docker..."
    docker compose -f docker-compose-jetson.yml up -d

    echo ""
    echo "============================================="
    echo "  ezlocalai Jetson Docker Deployment Complete!"
    echo "============================================="
    echo ""
    ...
    echo "  cd ${INSTALL_DIR}"
    echo "  docker compose -f docker-compose-jetson.yml up -d      # Start"
    echo "  docker compose -f docker-compose-jetson.yml down        # Stop"
    echo "  docker compose -f docker-compose-jetson.yml logs -f     # Follow logs"
    echo "  docker compose -f docker-compose-jetson.yml up -d --build  # Rebuild & start"
    echo ""
...
# ------------------------------------------------------------------
# 4. Create / update virtual environment (non-Jetson path)
# ------------------------------------------------------------------
